// Phone Guard Store — public catalog
const IMG_VERSION = 'v1';
const API_BASE = 'https://phoneguardstore-api.stawisystems.workers.dev';
(async function () {
  ...
  const NEW_DAYS = 7;
  const LOW_STOCK = 3;
  const WISHLIST_KEY = 'pgs_wishlist';
  let items = [];
  let settings = {};
  let suspended = false;
  ...
  const ANALYTICS_KEY = 'pgs_analytics';
  ...
  function enquireBody(item, soldOut, selectedSize) {
    ...
    let sizePart = '';
    if (!soldOut) {
      if (selectedSize) sizePart = ` (${selectedSize})`;
      else if (avail.length === 1) sizePart = ` (${avail[0]})`;
    ...
    return soldOut
      ? `Hi Phone Guard Store! I saw *${item.name}* is sold out. Will it be back in stock? Please let me know.`
      : `Hi Phone Guard Store! I'd like to check availability of *${item.name}*${sizePart}${pricePart} from your catalog.`;
  }
  ...
  function buildCatPills() {
    ...
    const groups = [{ label: null, options: [{ val: 'all', text: 'All phone types' }].concat(cats.map(c => ({ val: c, text: c }))) }];
    catPills.innerHTML = dropdownHTML({ kind: 'cat', value: currentCat, ariaLabel: 'Filter by phone type', groups });
  }
  ...
  // Root is bare .cdrop (this catalog's .filter-select is a full select style, so
  // it goes on the trigger button, not the positioning container).
  function dropdownHTML({ kind, value, ariaLabel, groups }) {
    ...
    return `<div class="cdrop${active ? ' cdrop--active' : ''}" data-kind="${kind}" aria-label="${escapeHtml(ariaLabel)}">`
      + `<button type="button" class="cdrop-trigger filter-select" aria-haspopup="listbox" aria-expanded="false"><span class="cdrop-current">${escapeHtml(cur.text)}</span></button>`
  ...
  function buildSizePills() {
    ...
    const grouped = {};
    sizes.forEach(s => { (grouped[sizeBrandGroup(s)] = grouped[sizeBrandGroup(s)] || []).push(s); });
    const groups = [{ label: null, options: [{ val: 'all', text: 'All models' }] }];
    order.forEach(g => {
      if (!grouped[g] || !grouped[g].length) return;
      groups.push({ label: g, options: grouped[g].map(s => ({ val: s, text: s })) });
    });
    sizePills.innerHTML = dropdownHTML({ kind: 'size', value: currentSize, ariaLabel: 'Filter by phone model', groups });
  }
  ...
  const IG_SVG = `<svg class="ig-icon" viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M21 12a9 9 0 1 1-2.64-6.36"></path><polyline points="21 3 21 9 15 9"></polyline></svg>`;
  ...
  function render() {
    ...
    gallery.innerHTML = visible.map(item => {
      ...
      const sizesHtml = avail.length
        ? `<div class="size-chips${pickRequired ? ' pickable' : ''}">${avail.map(s => `<button type="button" class="size-chip" data-size="${escapeHtml(s)}">${escapeHtml(s)}</button>`).join('')}${pickRequired ? '<span class="size-hint">Pick your model first</span>' : ''}</div>`
        : '';
      ...
      <article class="card ${soldOut ? 'sold' : ''}">
        ...
        <div class="card-body">
          ...
          <div class="card-price-row">
            ...
            ${avail.length ? '<a class="size-guide-link" data-action="size-guide">Which fits my device?</a>' : ''}
          </div>
  ...
  gallery.addEventListener('click', async e => {
    ...
    if (enquire) {
      ...
      if (!soldOut && avail.length > 1) {
        ...
        if (!selected) {
          e.preventDefault();
          showToast('Pick your model first to continue');
  ...
  function showSuspended() {
    ...
    const css = ('@keyframes pgSusFade{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:none}}'
      + '#suspendedOverlay{position:fixed;inset:0;z-index:99999;background:radial-gradient(ellipse at top,#122a43 0%,#0a1726 65%);color:#eef2f7;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:40px 24px;font-family:Inter,system-ui,-apple-system,sans-serif;animation:pgSusFade 0.65s ease both;}'
      + '#suspendedOverlay .pg-logo{width:140px;height:140px;border-radius:50%;object-fit:cover;background:#fff;border:2px solid #f5b50a;box-shadow:0 0 36px rgba(245,181,10,0.4),inset 0 0 0 1px rgba(255,255,255,0.04);margin-bottom:26px;}'
      + '#suspendedOverlay .pg-name{font-family:\'Cormorant Garamond\',Georgia,serif;font-size:34px;color:#ffd24d;letter-spacing:2.5px;font-weight:500;line-height:1;margin-bottom:8px;}'
      + '#suspendedOverlay .pg-tag{font-size:12px;color:#f5b50a;letter-spacing:2px;text-transform:uppercase;margin-bottom:30px;opacity:0.9;}'
      + '#suspendedOverlay .pg-rule{width:54px;height:1px;background:linear-gradient(90deg,transparent,#f5b50a,transparent);margin-bottom:30px;}'
      + '#suspendedOverlay .pg-head{font-family:\'Cormorant Garamond\',Georgia,serif;font-weight:500;font-size:clamp(30px,5vw,44px);margin:0 0 16px;color:#eef2f7;line-height:1.15;}'
      + '#suspendedOverlay .pg-body{font-size:16px;max-width:460px;line-height:1.65;opacity:0.82;margin:0 0 14px;}'
      + '#suspendedOverlay .pg-offer{font-size:16px;max-width:460px;line-height:1.6;margin:0 0 30px;color:#ffd24d;}'
      + '#suspendedOverlay .pg-offer b{color:#fff;font-weight:700;}'
      + '#suspendedOverlay .pg-ig{display:inline-flex;align-items:center;gap:10px;background:#f5b50a;color:#0a1726;padding:14px 30px;border-radius:999px;text-decoration:none;font-weight:600;font-size:15px;letter-spacing:0.3px;box-shadow:0 6px 24px rgba(245,181,10,0.3);transition:transform 0.2s ease,box-shadow 0.2s ease,background 0.2s ease;}'
      + '#suspendedOverlay .pg-ig:hover{background:#ffd24d;transform:translateY(-1px);box-shadow:0 8px 28px rgba(245,181,10,0.42);}'
      + '@media (max-width:480px){#suspendedOverlay .pg-logo{width:118px;height:118px;margin-bottom:22px;}#suspendedOverlay .pg-name{font-size:28px;letter-spacing:2px;}#suspendedOverlay .pg-tag{font-size:11px;margin-bottom:24px;}}'
    );
    ...
    o.innerHTML = (
      '<img class="pg-logo" src="' + logoUrl + '" alt="' + shopName + '">'
      ...
      + '<p class="pg-offer">Now you can <b>own this shop outright for a one-time Ksh 20,000</b>, no monthly fees. New stock you post on Instagram pulls straight into your shop. Buyers order on WhatsApp, and can filter by category and size to find what they want fast.</p>'
